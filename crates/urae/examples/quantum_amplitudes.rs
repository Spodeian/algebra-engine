...
fn main() {
    ...
    let s_12_re = ang_12.0 * sq_21.0 - ang_12.1 * sq_21.1;
    println!("Mandelstam s_12 = <1 2> [2 1] = {}", s_12_re);

    // 3. Parke-Taylor MHV Amplitude
    let s3 = WeylSpinor::from_null_momentum(&[100.0, 100.0, 0.0, 0.0]);
    let s4 = WeylSpinor::from_null_momentum(&[100.0, -100.0, 0.0, 0.0]);
    let spinors = vec![s1, s2, s3, s4];
    let amp = parke_taylor_mhv!(&spinors, 0, 1);
