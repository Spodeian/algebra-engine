...
fn test_wave_dalembert_exact_symbolic() {
    ...
    let sol = graph
        .solve_pde_wave_dalembert(f_init, None, c_node, x_sym, t_sym)
        .unwrap();
    ...
    assert!(
        (val - 13.0).abs() < 1e-9,
        "d'Alembert wave at (3, 1) should be 13, got: {} (expression: {})",
        val,
        sol_str
    );
}
...
fn test_wave_separated_modes() {
    ...
    assert!(
        x_str.contains("sin"),
        "Spatial mode must be sinusoidal, got: {}",
        x_str
    );
    assert!(
        t_str.contains("cos"),
        "Temporal mode must be oscillatory cos, got: {}",
        t_str
    );
}
...
fn test_laplace_separated_harmonic_modes() {
    ...
    assert!(
        x_str.contains("sin"),
        "Laplace X mode must be sin, got: {}",
        x_str
    );
    assert!(
        y_str.contains("sinh"),
        "Laplace Y mode must be hyperbolic sinh, got: {}",
        y_str
    );
}
...
fn test_transport_characteristic_solution() {
    ...
    let sol = graph
        .solve_pde_transport_1d(f_init, c_node, x_sym, t_sym)
        .unwrap();
    ...
    assert!(
        (val - 1.0).abs() < 1e-9,
        "Transport characteristic at (7, 2) should be 1, got: {}",
        val
    );
}
...
fn test_radial_bessel_eigenmode_and_evaluation() {
    ...
    assert!(
        uni_str.contains("J_0"),
        "Unicode should display J_0, got: {}",
        uni_str
    );
    assert!(
        lat_str.contains("J_{0}"),
        "LaTeX should display J_{{0}}, got: {}",
        lat_str
    );
...
fn test_executor_pde_dispatch() {
    ...
    assert!(
        !res.is_error,
        "Execution of PdeOpKind::Wave1D must succeed: {:?}",
        res.error_msg
    );
    assert!(
        res.output_text
            .contains("Wave Equation d'Alembert Solution")
    );
    ...
    assert!(
        !res_bessel.is_error,
        "Execution of PdeOpKind::RadialBessel must succeed: {:?}",
        res_bessel.error_msg
    );
