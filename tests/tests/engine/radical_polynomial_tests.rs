...
fn test_binomial_radicals_cubic() {
    ...
    assert!(
        found_two,
        "Cubic binomial x^3 - 8 must have exact integer root 2"
    );
}
...
fn test_binomial_radicals_quartic() {
    ...
    assert_eq!(
        roots.len(),
        4,
        "Quartic binomial x^4 - 16 must yield 4 roots: 2, -2, 2i, -2i"
    );
    let unicode_fmt = UnicodeFormatter;
    let root_strs: Vec<String> = roots
        .iter()
        .map(|&r| unicode_fmt.format(&graph, r).unwrap())
        .collect();
...
fn test_biquadratic_nested_radicals() {
    ...
    assert_eq!(
        roots.len(),
        4,
        "Biquadratic x^4 - 5x^2 + 4 must yield exactly 4 exact integer roots"
    );
...
fn test_biquadratic_irrational_nested_radicals() {
    ...
    assert_eq!(
        roots.len(),
        4,
        "Biquadratic x^4 - 2x^2 - 1 must produce 4 exact nested radical roots"
    );
}
...
fn test_cardano_cubic_rational_factorization() {
    ...
    assert_eq!(
        roots.len(),
        3,
        "Factored cubic x^3 - 6x^2 + 11x - 6 must yield {{1, 2, 3}}"
    );
    let mut vals = Vec::new();
    for &r in &roots {
        if let ExprKind::Number(Number::Integer(i)) = graph.get(r).kind {
...
fn test_bring_radical_exact_and_numerical() {
    ...
    assert_eq!(
        roots.len(),
        1,
        "Bring radical quintic must yield exact BR(1) root"
    );
    let unicode_fmt = UnicodeFormatter;
    ...
    assert!(
        br_str.contains("BR"),
        "Unicode representation should display BR(1), got: {}",
        br_str
    );

    let latex_str = latex_fmt.format(&graph, roots[0]).unwrap();
    assert!(
        latex_str.contains("\\operatorname{BR}"),
        "LaTeX should display \\operatorname{{BR}}, got: {}",
        latex_str
    );
    ...
    assert!(
        (num_val - (-0.7548776662)).abs() < 1e-6,
        "Numerical evaluation of BR(1) was {}",
        num_val
    );
    ...
    assert!(
        residual.abs() < 1e-9,
        "Residual of Bring radical root must be < 1e-9, got: {}",
        residual
    );
}
...
fn test_higher_degree_rootof_fallback() {
    ...
    assert_eq!(
        roots.len(),
        6,
        "Degree 6 polynomial must return 6 exact RootOf branches"
    );
    let unicode_fmt = UnicodeFormatter;
    let r0_str = unicode_fmt.format(&graph, roots[0]).unwrap();
    assert!(
        r0_str.contains("RootOf"),
        "Must represent unsolved algebraic roots symbolically as RootOf, got: {}",
        r0_str
    );
}
